
import gc, torch
import numpy as np
from tqdm import trange
from pykeops.torch import LazyTensor
from sklearn.decomposition import PCA
import matplotlib.pyplot as plt; plt.ion()

np.random.seed(42); torch.manual_seed(42)

x, y = torch.load('mnist.pth', weights_only=False)
x = x[:10000]
y = y[:10000]
n = len(x)

k = 15

x_cuda = torch.tensor(x).to("cuda").contiguous()
dists = (LazyTensor(x_cuda[:, None]) - LazyTensor(x_cuda[None])).square().sum(-1)
knn_idx = dists.argKmin(K=k + 1, dim=0)[:, 1:].cpu()
del x_cuda, dists

r, c = torch.triu_indices(n, n, 1)

nn = torch.zeros(n, n).to("cuda")
nn[torch.arange(n)[:, None], knn_idx] = 1
A = (nn + nn.T).clip(0, 1)

n_bar = 2*15*5/1.5
eps = n_bar / (2*n)

I = torch.eye(n).to("cuda")
O = torch.ones(n, n).to("cuda")
H = I - O/n
L = A.sum(0).diag() - A

u, v = torch.triu_indices(A.size(0), A.size(0), 1, device=A.device)
e = A[u,v].bool()
u, v = u[e], v[e]
In = torch.zeros(A.size(0), u.numel(), device=A.device, dtype=A.dtype)
i = torch.arange(u.numel(), device=A.device)
In[u, i] = 1
In[v, i] = -1
# In = In.double()
# In = In @ torch.randn_like(In).T / np.sqrt(n)
# Lt = (In @ In.T).float()

_l, _U = torch.linalg.eigh(L/n)
init = _U[:, [1, 2]] @ _l[[1, 2]].reciprocal().sqrt().diag()
model = torch.nn.Embedding.from_pretrained(init.clone().normal_(), freeze=False).to("cuda")

def bound_interp():
    X = model.weight - model.weight.mean(0)
    D = (X[:, None] - X[None]).square().sum(-1)
    p = 1/(1 + D)
    S = I + eps*(H @ p @ H) + 2*eps*X@X.T # Phi @ Phi.T
    return (I + S @ L).logdet()/(2*eps) - n*(S).logdet()

optimizer = torch.optim.SGD(model.parameters(), weight_decay=0.0, lr=0.01)
for epoch in (bar := trange(25, leave=False)):
    # optimizer.param_groups[0]["lr"] = 1 - epoch/25
    model.train()
    loss = bound_interp()
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
    bar.set_description(f"L:{loss.item()}")
    gc.collect(); torch.cuda.empty_cache()

embd = model.weight.detach().cpu().numpy()

plt.scatter(*embd.T, c=y, alpha=0.75, s=1, cmap="tab10", edgecolor="none")
plt.axis('off')
plt.tight_layout()
